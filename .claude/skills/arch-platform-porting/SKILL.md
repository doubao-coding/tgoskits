...
- **Guest emulation and DMA mapping**: when an FDT-described passthrough device overlaps an emulated MMIO resource, keep the firmware node but punch the emulated range out of the stage-2 passthrough mapping so the emulator receives accesses. Physical passthrough devices that perform DMA require guest RAM backed by an identity mapping or an active IOMMU translation; `MAP_ALLOC` RAM alone only translates CPU accesses and is not sufficient for device DMA.
