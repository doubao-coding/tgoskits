...
// Multi-mutex PI: releasing a mutex with no waiters must NOT clear the donation
// contributed by a waiter still blocked on another held mutex.
static MULTI_LOW_LOCKED: AtomicBool = AtomicBool::new(false);
static MULTI_HIGH_WAITING_ON_A: AtomicBool = AtomicBool::new(false);
static MULTI_LOW_PRIO_AFTER_B_RELEASE: AtomicIsize = AtomicIsize::new(isize::MIN);
static MULTI_HIGH_DONE: AtomicBool = AtomicBool::new(false);
...
fn run_mutex_donation_survives_other_mutex_release_test() -> crate::TestResult {
    let mutex_a = Arc::new(Mutex::new(()));
    let mutex_b = Arc::new(Mutex::new(()));

    raise_current_priority(50);
    let low = spawn_multi_lock_low(mutex_a.clone(), mutex_b.clone());
    raise_current_priority(0);
    while !MULTI_LOW_LOCKED.load(Ordering::Acquire) {
        thread::yield_now();
    }

    raise_current_priority(50);
    let high = spawn_multi_lock_high(mutex_a.clone());
    raise_current_priority(0);

    // `low` holds both A and B. `high` (priority 30) blocks on A and donates
    // 30 to `low`. `low` then releases B (which has no waiters) and records
    // its own effective priority: with correct per-mutex recomputation it stays
    // at 30 (the donation from A survives releasing B); a blind
    // clear-on-unlock would drop it to the base priority 10. No interference
    // task is needed -- the priority value itself is the assertion.
    let mut prio_after_b = isize::MIN;
    for _ in 0..10_000 {
        prio_after_b = MULTI_LOW_PRIO_AFTER_B_RELEASE.load(Ordering::Acquire);
        if prio_after_b != isize::MIN {
            break;
        }
        thread::yield_now();
    }
    if prio_after_b != 30 {
        return Err(
            "releasing an uncontended mutex cleared the donation from another still-held mutex",
        );
    }

    while !MULTI_HIGH_DONE.load(Ordering::Acquire) {
        thread::yield_now();
    }
    low.join();
    high.join();

    std::println!("sched-rt-fifo multi-mutex donation survives OK");
    Ok(())
}

fn spawn_multi_lock_low(mutex_a: Arc<Mutex<()>>, mutex_b: Arc<Mutex<()>>) -> ax_task::AxTaskRef {
    let task = TaskInner::new(
        move || {
            let guard_a = mutex_a.lock();
            let _guard_b = mutex_b.lock();
            MULTI_LOW_LOCKED.store(true, Ordering::Release);
            assert!(
                ax_task::set_priority(0),
                "multi-PI low task failed to lower base priority"
            );
            while !MULTI_HIGH_WAITING_ON_A.load(Ordering::Acquire) {
                thread::yield_now();
            }
            // Release the mutex with NO waiters (B). The donation from A's
            // waiter must survive this release.
            drop(_guard_b);
            MULTI_LOW_PRIO_AFTER_B_RELEASE.store(
                ax_task::current().sched_priority() as isize,
                Ordering::Release,
            );
            // Release A, handing it to the high-priority waiter, then exit.
            drop(guard_a);
        },
        "rt-fifo-multi-low".into(),
        ax_task::default_task_stack_size(),
    );
    ax_task::spawn_task_with(task, |task| task.set_sched_priority(10))
}

fn spawn_multi_lock_high(mutex_a: Arc<Mutex<()>>) -> ax_task::AxTaskRef {
    let task = TaskInner::new(
        move || {
            MULTI_HIGH_WAITING_ON_A.store(true, Ordering::Release);
            let _guard = mutex_a.lock();
            MULTI_HIGH_DONE.store(true, Ordering::Release);
        },
        "rt-fifo-multi-high".into(),
        ax_task::default_task_stack_size(),
    );
    ax_task::spawn_task_with(task, |task| task.set_sched_priority(30))
}
